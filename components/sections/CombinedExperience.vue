...
const projectsTarget = 350; // Total projects completed
const duration = 2000;
...
onMounted(() => {
  ...
  const yearsInterval = setInterval(() => {
    years.value += yearsIncrement;
    if (years.value >= target) {
      years.value = target;
  ...
  const projectsInterval = setInterval(() => {
    projects.value += projectsIncrement;
    if (projects.value >= projectsTarget) {
      projects.value = projectsTarget;
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <p class="text-lg mb-6">
            At Cambridge Construction, our team brings decades of expertise to every project. Our combined knowledge and experience ensure each build benefits from time-tested wisdom and innovative approaches.
          </p>
          <div class="border-l-2 border-slate-400 pl-4 mt-8">
            <p class="text-lg mb-4">
              Our leadership team includes seasoned project managers, master builders, award-winning architects, and specialized engineers, creating a powerhouse of construction knowledge under one roof.
            </p>
            <p class="text-lg">
              From historic renovations to cutting-edge commercial spaces, our collective expertise allows us to tackle any challenge with confidence and precision.
            </p>
          </div>
          <button
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div class="bg-slate-800 p-8 shadow-lg border-l-4 border-slate-400 relative overflow-hidden">
            <ChartPieIcon class="size-20 text-slate-600 absolute -bottom-2 -right-2 opacity-30" />
            <h3 class="text-2xl font-bold mb-2">Years of Experience</h3>
            <p class="text-6xl font-bold mb-2">{{ Math.floor(years) }}+</p>
            <p class="text-slate-400">Combined industry expertise</p>
          </div>
          <div class="bg-slate-800 p-8 shadow-lg border-l-4 border-slate-400 relative overflow-hidden">
            <ChartPieIcon class="size-20 text-slate-600 absolute -bottom-2 -right-2 opacity-30" />
            <h3 class="text-2xl font-bold mb-2">Projects Completed</h3>
